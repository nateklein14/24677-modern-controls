...
class EKF_SLAM():
    ...
    def _f(self, x, u):
        ...
        x_next[2] = wrapToPi(psit+self.dt*u[2])
    ...
    def _h(self, x):
        ...
        distances = np.sqrt((landmarks_x - x[0])**2 + (landmarks_y - x[1])**2)
        headings = wrapToPi(np.arctan2(landmarks_y - x[1], landmarks_x - x[0]) - x[2])

        y = np.concatenate((distances, headings), 0)
    ...
    def _compute_F(self, x, u):
        ...
        F = np.eye(len(x), len(x))
        F[0,2] = self.dt*(-u[0]*np.sin(x[2])-u[1]*np.cos(x[2]))
        F[1,2] = self.dt*(u[0]*np.cos(x[2])-u[1]*np.sin(x[2]))
    ...
    def _compute_H(self, x):
        ...
        H = np.zeros((2*self.n, 3+2*self.n), dtype=float)
        ...
        distances = np.sqrt((landmarks_x - X)**2 + (landmarks_y - Y)**2)

        for i in range(self.n):
            mix = landmarks_x[i]
            miy = landmarks_y[i]
            H[i, 0] = -(mix - X) / distances[i]
            H[i, 1] = -(miy - Y) / distances[i]
            H[i, 3+2*i] = (mix - X) / distances[i]
            H[i, 3+2*i+1] = (miy - Y) / distances[i]

            H[i+self.n, 0] = (miy - Y) / (distances[i]**2)
            H[i+self.n, 1] = -(mix - X) / (distances[i]**2)
            H[i+self.n, 3+2*i] = -(miy - Y) / (distances[i]**2)
            H[i+self.n, 3+2*i+1] = (mix - X) / (distances[i]**2)
            H[i+self.n, 2] = -1
