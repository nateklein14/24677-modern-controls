...
class AStar(object):
    ...
    def heuristic(self, current, goal):
        ...
        Euclidean distance
        """
        return ((goal.x - current.x)**2 + (goal.y - current.y)**2)**(1/2)
    ...
    def plan(self, start_ind, goal_ind):
        ...
        start_node.f_value = start_node.h_value + start_node.g_value
        """
        END TODO
        """
        ...
        while len(open_list):
            ...
            for successor in self.get_successor(current):
                ...
                d = ((current.x - successor.x)**2 + (current.y - successor.y)**2)**(1/2)
                successor.g_value = current.g_value + d
